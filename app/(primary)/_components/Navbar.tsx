...
const Navbar = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-30 transition-all bg-black font-bold  ${
        bg ? "bg-black shadow-lg lg:h-[8rem] h-[5rem]" : "bg-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6  lg:px-8 flex items-center justify-between lg:gap-[12rem] xl:justify-start lg:h-32 h-[5rem]">
        <Link href="/" className="hidden lg:block">
          <div className="flex-shrink-0">
            <Image src={Logo} alt="Jacinhomes Logo" width={200} height={200} />
          </div>
        </Link>
        ...
        <nav className="hidden xl:flex space-x-[2rem] items-center  text-light">
          <Link href="/" className={`link ${pathname === "/" ? "active text-red-600" : ""}`}>
            Home
          </Link>
          <Link href="/aboutus" className={`link ${pathname === "/aboutus" ? "active text-red-500" : ""}`}>
            About Us
          </Link>
      ...
      {menuOpen && (
        <div className="xl:hidden flex flex-col justify-center items-center text-center bg-white text-black  p-4">
          ...
          <Link href="/aboutus" className={`block py-2 ${pathname === "/aboutus" ? "active text-red-500" : ""}`}>
            About Us
          </Link>
